		...
		Scanner sc = new Scanner(System.in);			
			...
			while(true) {
				...
				if(sel == 1) {
					...
					if(log==1) {
						...
						while(run) {												//이부분놓침
							...
							if(buyI>=0&&buyI<items.length) {						//이부분놓침
								jang[count][0]=log;									//
								jang[count][1]=buyI;								//0//1//2 아래에서 인덱스로 활용?
								count++;
							}							
						}
					}else {
					...
					int myitem[]=new int[items.length];				//{0,0,0}?
					
					if(log!=-1) {
					for(int i=0;i<count;i++) {
						if(jang[i][0]==log) {
							myitem[jang[i][1]]++;		//jang[i][1]은 sel==3(쇼핑)에서 내가산 items배열의 인덱스정보
						}
					}
